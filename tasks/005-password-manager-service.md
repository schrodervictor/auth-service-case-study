...
hash, constant-time comparison via timingSafeEqual, and clear separation
between the interface and injectable implementation.
...
3. **Scrypt parameters** — Use Node.js defaults for crypto.scrypt (N=16384,
   r=8, p=1). Derived key length: 64 bytes. Salt length: 32 bytes (generated
   via crypto.randomBytes). These are secure defaults and match the Node.js
...
- Takes a plaintext password, generates a random 32-byte salt, derives a
  64-byte key via scrypt, returns "salt-hex.key-hex".
...
- Constants like SALT_LENGTH and KEY_LENGTH should be private module-level
  const values (not magic numbers in the methods).
- The interface must be exported as a named export (not default). The impl
  class must also be a named export.
...
PasswordManagerServiceImpl from ./services, and uncomment the container
binding: container.bind<PasswordManagerService>(TYPES.PasswordManagerService)
...
All tests are unit tests — this service has no external dependencies (no DB,
no network). Tests go in tests/unit/services/password-manager-service.test.ts.
...
6. Empty string password still produces a valid hash (scrypt handles empty
   input — input validation belongs in UserService, not here)
...
Use import 'reflect-metadata' at the top of the test file (needed for
Inversify decorators). Import the impl class directly for most tests — no need
to go through the DI container except for the DI integration tests.
...
- **Corrupted stored hash**: compare must return false (not throw) when the
  stored password format is invalid. This handles DB corruption gracefully.
  Wrap the parsing/comparison in a try-catch and return false on any error.
...
No new npm packages. crypto and util are Node.js built-ins. inversify is
already installed.
...
- **Acceptance Criteria**:
  - [ ] src/services/password-manager-service.ts exports PasswordManagerService
        interface and PasswordManagerServiceImpl class
  - [ ] toHash generates a random 32-byte salt, derives a 64-byte key via
        scrypt, returns "salt-hex.key-hex"
  - [ ] compare parses the stored hash, re-derives the key, uses
        timingSafeEqual for comparison
  - [ ] compare returns false (not throws) for malformed stored passwords
  - [ ] Constants SALT_LENGTH and KEY_LENGTH are used (no magic numbers)
  - [ ] @injectable() decorator is applied to the impl class
  - [ ] src/services/index.ts exports the service
- **Status**: pending
...
- **Acceptance Criteria**:
  - [ ] TYPES.PasswordManagerService symbol is active in src/lib/types.ts
  - [ ] createContainer binds PasswordManagerService to
...
- **Description**: Comprehensive unit tests for both toHash and compare, plus
  DI integration verification
- **Acceptance Criteria**:
  - [ ] tests/unit/services/password-manager-service.test.ts exists with all
        test cases from the test strategy
  - [ ] Tests cover: format validation, salt uniqueness, round-trip
        correctness, wrong password rejection, corrupted hash handling, special
        characters
  - [ ] DI integration test verifies TYPES.PasswordManagerService is bound
  - [ ] make test-unit passes with all tests green
  - [ ] No tests mock the crypto module (real hashing is tested)
- **Status**: pending
